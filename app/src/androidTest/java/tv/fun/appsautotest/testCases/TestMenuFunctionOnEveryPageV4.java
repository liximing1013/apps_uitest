...
public class TestMenuFunctionOnEveryPageV4 extends CommonMethod {
    ...
    public void LC_MENU_01_SearchTabMenuOperation() {
        try {
            ...
            pressLeft(2);
            systemWait(WAIT);
            if(uiDevice.hasObject(By.text("搜索片名、演员、导演").res("com.bestv.ott:id/search"))) {
                uiDevice.pressMenu();
                systemWait(3);
                if (uiDevice.hasObject(By.text("搜索"))) {
                    uiDevice.pressDPadCenter();
                    systemWait(WAIT);
                    UiObject2 SearchPage = uiDevice.findObject(By.text("用手机搜片"));
                    Assert.assertEquals("跳转搜索页面错误","用手机搜片",SearchPage.getText());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void LC_MENU_02_TVTabMenuOperation() {
        try {
            ...
            if(tabName.isSelected() && uiDevice.hasObject(By.text("添加"))){
                uiDevice.pressMenu();
                systemWait(WAIT);
                ...
                if(Menu != null) {
                    ...
                    uiDevice.pressDPadCenter();
                    systemWait(WAIT);
                    UiObject2 page = uiDevice.findObject(By.text("设置"));
    ...
    public void LC_MENU_03_VideoTabMenuOperation() {
        try {
            ...
            List<UiObject2> menuList = uiDevice.findObjects(By.res("android:id/tv_fun_menu_text"));
            if(menu != null && menuList.size() == 3) {
                pressRight(1);
                uiDevice.pressDPadCenter();
                systemWait(WAIT);
                UiObject2 page = uiDevice.findObject(By.text("视频分类"));
                m_Actual = page.getText();
                m_Expect = "视频分类";
                m_Pass = m_Actual.equalsIgnoreCase(m_Expect);
